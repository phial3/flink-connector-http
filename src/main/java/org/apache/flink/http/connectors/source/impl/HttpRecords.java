 ...
 */
@Internal
public final class HttpRecords<T> implements RecordsWithSplitIds<RecordsAndPosition<T>> {
    ...
    @Nullable
    @Override
    public RecordsAndPosition<T> nextRecordFromSplit() {
    ...
    public static <T> HttpRecords<T> forRecords(
            final String splitId, final BulkFormat.RecordIterator<T> recordsForSplit) {
        return new HttpRecords<>(splitId, recordsForSplit, Collections.emptySet());
    }

    public static <T> HttpRecords<T> finishedSplit(String splitId) {
        return new HttpRecords<>(null, null, Collections.singleton(splitId));
    }
}
